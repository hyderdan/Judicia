  ...
  const handleAnalyze = async () => {
    ...
    try {
      const res = await fetch(
        `http://127.0.0.1:8000/evidence/${selectedEvidenceId}/analyze`,
        { method: "POST" }
      );

      const data = await res.json();

      setEvidenceDetail(prev => ({
        ...prev!,
          ...
          <div className="flex-1 flex flex-col items-center justify-center">
            {isAnalyzing || evidenceDetail?.analysis_status === 'processing' ? (
              ...
              <div className="w-full">
                <div className="bg-background/60 border rounded-xl p-6 text-sm leading-relaxed whitespace-pre-line">
                  {evidenceDetail.analysis_report}
                </div>
              </div>
